...
class XxeAdvancedChecks:
    ...
    def inject_file(request):
        """function check if the attacker tries to inject some malicious file into the server
        it checks it with the list of malicious file extensions
        :param request: the user's request
        :type request: string
        return: the estimated risk level
        :rtype: risk level enum"""
