...
functions = []
for xd in dicti:
    xd["rule"] = xd["rule"].replace('"', '\\"')
for noder in dicti:
    if int(noder["impact"]) == 2:
        functions.append(f'def  {"_".join(noder["description"].split(" ")[2:4])}(request):\n\t"""\n\tfunction {noder["description"]}\n\t{" ".join(noder["tags"]["tag"])}\n\t"""\n\treturn RiskLevels.NEGLIGIBLE if\\\n\tre.search(r"{noder["rule"]}", request)\\\n\telse RiskLevels.NO_RISK')
    elif 3<=int(noder["impact"])<=4:
        functions.append(f'def  {"_".join(noder["description"].split(" ")[2:4])}(request):\n\t"""\n\tfunction {noder["description"]}\n\t{" ".join(noder["tags"]["tag"])}\n\t"""\n\treturn RiskLevels.SLIGHT if\\\n\tre.search(r"{noder["rule"]}", request)\\\n\telse RiskLevels.NO_RISK')
    elif 5<=int(noder["impact"])<=6:
        functions.append(f'def  {"_".join(noder["description"].split(" ")[2:4])}(request):\n\t"""\n\tfunction {noder["description"]}\n\t{" ".join(noder["tags"]["tag"])}\n\t"""\n\treturn RiskLevels.MODERATE if\\\n\tre.search(r"{noder["rule"]}", request)\\\n\telse RiskLevels.NO_RISK')
    else:
        functions.append(f'def  {"_".join(noder["description"].split(" ")[2:4])}(request):\n\t"""\n\tfunction {noder["description"]}\n\t{" ".join(noder["tags"]["tag"])}\n\t"""\n\treturn RiskLevels.CRITICAL if\\\n\tre.search(r"{noder["rule"]}", request)\\\n\telse RiskLevels.NO_RISK')
for some in functions:
    print("@staticmethod")
    print(some)