...
deep_info = {
    ...
    "check_user_disclosure": "the user table is a global table in every mySql database, so we make sure the attacker"
                             "doesn't try to access it and get the information about the users\n",

    "check_mongo_db_command": "the attacker may use some of the common commands in mongoDB, that are not so\n"
                              "frequent, so it is suspicious he use this characters\n",

    "check_cstyle_comment": "the user is trying to add some Cstyle comments into your sql query maybe in order\n"
                            "to delete the info after the comments\n",

    "check_blind_benchmark": "blind sql injection are the type that doesn't shown screen, attacker\n"
    ...
    "check_load_file_disclosure": "load file function generally Reads the file and returns the "
                                  "file contents as a string\n"
                                  "it can be harmful when the user load some "
                                  "important file and then select it with a\n"
                                  "regular query\n",

    "check_load_data_disclosure": "The LOAD DATA statement reads rows from a text file into a table\n"
                                  "attacker can use this function in order to inject some "
                                  "malicious script into the database\n",

    "check_write_into_outfile": "this function writes the selected rows to a file, so the attacker can write some\n"
                          "parts of the database to a specific file and then some how take it\n",

    "check_blind_sql_sleep": "with this function the attacker can delayed the server's response and see if his\n"
                             "injections work\n",

    "check_concat_command": "the concat command is another way for the attacker to check if his injection can work\n"
                            "this command is basically concat couple of strings together into one string\n",

    "check_information_disclosure": "INFORMATION_SCHEMA provides access to database metadata.\n"
    ...
    "check_sleep_pg_command": "with this function the attacker can delayed the\n"
                              "server's response and see if his injections work\n",

    "check_blind_tsql": "with this function the attacker can delayed the server's response and see if his\n"
                                "injections work\n",

    "check_substr_command": "the substr function is not so dangerous for its self,\n"
    ...
    "check_user_command": "the user function can allow the attacker to manipulate the users\n"
                          "permissions and also see all users exist in\n"
                          "the database and more info about the users\n",

    "check_version_command": "the version function seems very nice and harmless "
                             "but you should be careful\n"
                             "the attacker can see the version of the database and this info\n"
                             "can be very useful for the next steps of the attack\n",

    "check_system_variable": "system variables that store information about the running "
                             "package and its objects\n"
                             "this information can be very useful for future attacker",

    "check_if_command": "the if statement can use for check some condition in it,\n"
                        "with that said, attacker can use this statement to check\n",

    "check_ifnull_command": "the if statement can use for check some condition in it,\n"
                            "with that said, attacker can use this statement to check\n",

    "check_case_command": "the case statement check for each case if its true\n"
                          "just like the if condition, the case can show the attacker if the injection\n"
                          "work as he expected to\n",

    "check_exec_command": "the exec function is a very dangerous one, you should be careful\n"
                            "with this function the user can execute wide range of functions and scripts\n"
                            "on your server side\n",

    "check_create_procedure_command": "with the create function the attacker can create procedure or function\n"
                                      "that could run on your database.\n",

    "check_user_info_disclosure": "pg_user is a specific command that can return"
                                  " info about all the users\n"
                                  "in a specific database, in the wrong hands,"
                                  "this information about users can be dangerous\n",

    "check_db_info_disclosure": "pg_database stores information about the available databases\n"
                                "this information should not reach the attacker\n",

    "check_shadow_info_disclosure": "The view pg_shadow exists for backwards compatibility:\n"
                                    "it emulates a catalog that existed in PostgreSQL\n"
                                    "this option could be used by an attacker that runs his own catalog\n",

    "check_db_command": "The DATABASE() function returns the name of the current database.\n"
                            "this information can help the attacker so it should not reach the attacker\n",


    "sql or condition": "or is a logical statement, that can be very dangerous to your database\n"
                        "in order to make any query he wants, the attacker can add an or statement to the\n"
                        "query, that will be always true, like 1=1\n",
    "check_grant_revoke": "the grant or revoke functions are letting the attacker the option\n"
                          "of manipulate and change the permissions of the database\n",

    "check_alter": "with alter table function, the attacker can modify the database\n",

    "check_delete": "with delete function, the user can delete, rows, values and even database\n"
                        "that can be vary dangerous for the server\n",

    "check_update": "the update command just modify and change values appear in the database\n"
    ...
    "check_select_into": "function get and return values or raw from a database\n"
                         "user can use it to see secret information, that he shouldn't see\n"
                         "like credit cards number, that function can be very dangerous and"
                         " it is very common among sqlinjection hackers",

    "check_select_from": "function get and return values or raw from a database\n"
                         "user can use it to see secret information, that he shouldn't see\n"
                         "like credit cards number, that function can be very dangerous and"
                         "it is very common among sqlinjection hackers",

    "check_drop": "function drop can delete raw, table and even database, potential hacker can use it for\n"
