...
class BasicChecks:
    ...
    def alter(request):
        ...
        return RiskLevels.MODERATE \
            if re.search(r"""alter\s+table\s+.+?\s+(?:add|drop\s+column)\s+.+""",request) else RiskLevels.NO_RISK
    ...
    def drop(request):
        ...
        return RiskLevels.CATASTROPHIC \
            if re.search(r"""drop\s+(?P<deleteinfo>database|index|table|view)\s+.+""", request) else RiskLevels.NO_RISK
    ...
    def insert(request):
        ...
        return RiskLevels.CRITICAL if re.search(
            ...
            request) else RiskLevels.NO_RISK
    ...
    def or_statement(request):
        ...
        return RiskLevels.CRITICAL \
            if re.search(r"""or(?P<statement>(?:not\s+)*\s*(?P<operators>.+?<[^=>]+|[^=!<>]+=[^=]+|[^<]+?>[^=]+|.+?(?:==|<=|>=|!=|<>).+?)\s*|(?:not\s+)*.+?\s+(?:(?P<like>like\s+.+)|(?P<betweenand>between\s+.+?and\s+.+)|(?P<in>in\s*\(.+\))))""", request) \
            else RiskLevels.NO_RISK
